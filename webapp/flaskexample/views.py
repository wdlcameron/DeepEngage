...
def get_best_image(img_list, predictions, base_path = Path(r'flaskexample/static/temp'), img_name = 'best_image.png'):
...
def open_image_path(inp_path):
	if os.path.exists(inp_path):
		...
		base_path = Path(r'flaskexample/static/temp')
...
def make_composite_image(list_of_images, predictions = None, base_path = Path(r'flaskexample/static/temp')):
...
def make_optimized_image(original_image, optimized_image, filtered_images, time_recc = "Tuesday at 4", 
						filter_recc = 'ColorPop', predictions = None, base_path = Path(r'flaskexample/static/temp')):
...
def make_annotated_image(img, prediction = None, base_path = Path(r'flaskexample/static/temp')):
	main_image_name = 'predictions.png'
	alt_image_name = 'original_image.png'
	...
	if prediction is not None:
		ax.text(0.5, 0.5, prediction, horizontalalignment='center',  
		fontsize = 24, color = 'blue', verticalalignment='center', 
		backgroundcolor = 'white', transform=ax.transAxes)
...
DEMO_PATH = Path(r'flaskexample/demo_images')
DEMO_SINGLE_PATH = Path(r'flaskexample/single_images')
...
demo_groups = {'Demo_1':[DEMO_PATH/"_C1yuZv7hC.jpg",
				...
				'Demo_3':[DEMO_PATH/"Bh18vPxFhDp.jpg",
						DEMO_PATH/"B1wlC9mJVWA.jpg",
						DEMO_PATH/"BuZYOemne61.jpg",
						DEMO_PATH/"BO2Xf38Bv5_.jpg"],

				'Demo_4':[DEMO_PATH/"3fL3bFwCEz.jpg",
						DEMO_PATH/"4IX_MCP7qT.jpg",
						DEMO_PATH/"6PfEkCP7jB.jpg",
						DEMO_PATH/"6nVK-cv7jJ.jpg"]
				}
...
cache = {'Processed': False,
		...
		'single_images': [str(x.relative_to(DEMO_SINGLE_PATH)) for x in DEMO_SINGLE_PATH.ls() if x.suffix == '.jpg'],
           }


cache['single_images_len'] = len(cache['single_images'])





cache['learn'] = load_learner(r"flaskexample/models","Second Attempt.pkl")
...
def home_page():
	return render_template('index.html',
							single_image_list = cache['single_images'],
							single_image_length = cache['single_images_len'],
								)  # render a template
...
#  	 
   	# Pull input
	form_data =request.args.get('user_input')
	
	list_of_images = [DEMO_SINGLE_PATH/form_data]
	...
	result_path, alt_path = make_annotated_image(processed_images[0], predictions[0])
	...
	if some_input == '':
		...
		learn = cache['learn']
		image = get_image_from_url(some_input)
...
@app.route('/comp_output')
def comp_recommendation_output():
#  	 
   	# Pull input
